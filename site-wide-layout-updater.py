...
# Specific files to update inside /districts/folder/
DISTRICT_TARGET_PAGES = [
    'resources.html',
...
NEW_CSS = """
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Index Layouts (Grids & Ads) */
  .sales-card { background: linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%); color: white; padding: 35px; border-radius: 12px; text-align: center; margin: 40px 0; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); }
  .sales-card .badge { background: #d4af37; color: #000; padding: 5px 15px; font-weight: bold; border-radius: 20px; font-size: 0.8rem; }
...
def process_all_pages():
    ...
    print("--- 1. Updating Sub-District Internal Pages ---")
    if os.path.exists(DISTRICTS_DIR):
        for district_folder in os.listdir(DISTRICTS_DIR):
            ...
            for target_page in DISTRICT_TARGET_PAGES:
                filepath = os.path.join(district_path, target_page)
                if os.path.exists(filepath):
                    if update_file_layout(filepath):
                        updated_count += 1
                        
    print("\n--- 2. Updating Non-District & Root Pages ---")
    for directory in NON_DISTRICT_DIRS:
        if not os.path.exists(directory):
            continue
            
        # Iterate over all .html files directly inside this folder
        for filename in os.listdir(directory):
            filepath = os.path.join(directory, filename)
            
            # Skip subdirectories, process only HTML files
