...
def inject_silo_nav(soup, slug, title):
    """Finds or creates the silo-nav right below the h1 tag, using ABSOLUTE paths."""
    new_nav_html = f"""
    <div class="silo-nav" style="background-color: #e9ecef; padding: 14px 20px; border-radius: 8px; margin: 20px 0 30px; font-size: 15px; font-family: 'DM Sans', sans-serif; display: flex; flex-wrap: wrap; gap: 16px; align-items: center; border-left: 4px solid #6c757d;">
        ...
        <a href="/districts/{slug}/index.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">District Hub</a> •
        <a href="/districts/{slug}/ard-process-guide-{slug}.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">ARD Guide</a> •
        <a href="/districts/{slug}/evaluation-child-find-{slug}.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">Evaluations (FIE)</a> •
        <a href="/districts/{slug}/what-is-an-fie-{slug}.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">What Is an FIE?</a> •
        <a href="/districts/{slug}/how-to-request-evaluation.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">Request an Eval</a> •
        <a href="/districts/{slug}/grievance-dispute-resolution-{slug}.html" style="text-decoration: none; color: #2563eb; font-weight: 500;">Dispute Resolution</a>
    </div>
    """
...
def update_hub_page(file_path, slug, title, enrollment):
    """Updates the silo-grid, the silo-nav, cleans double headers, and injects enrollment."""
    ...
    # FIX: Remove double header (the redundant tailwind header) if it exists
    headers = soup.find_all('header')
    if len(headers) > 1:
        for header in headers:
            if "bg-white" in header.get("class", []) or "border-slate-200" in header.get("class", []):
                header.decompose()
...
def update_sub_page(file_path, slug, title):
    """Updates or injects the absolute silo-nav on all sub-article pages."""
    if not os.path.exists(file_path):
        return
