...
def process_district(district_slug, dry_run):
    ...
    # Inject before closing </div> of hub-grid
    # Find the hub-grid and insert as last card before </div>
    # Pattern: the last </a> before </div> in the hub-grid block
    hub_pattern = re.compile(
        r'(<div\s+class="hub-grid">.*?)(</div>)',
        re.DOTALL
    )
    match = hub_pattern.search(html)
    if not match:
        return "hub-grid not found"

    # Insert the new card just before the closing </div>
    new_hub = match.group(1) + "\n" + card + "\n" + match.group(2)
    new_html = html[:match.start()] + new_hub + html[match.end():]
