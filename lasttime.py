...
def fix_links_and_force_html(directory_path):
    """
    Scans HTML files to fix the FIE evaluation links. 
    It replaces the wrong city name with the folder name AND 
    ensures the link ends with .html.
    """
    ...
    # [^/"]+"                 -> Matches whatever is at the end of the filename (abilene-isd, abilene-isd.html, etc.) until the closing quote
    ...
    for root, _, files in os.walk(directory_path):
        for file in files:
            if file.endswith(".html"):
                ...
                # Replace the matched string and strictly enforce the .html extension
                ...
                if new_content != content:
                    with open(filepath, 'w', encoding='utf-8') as f:
                        f.write(new_content)
                    print(f"✅ Fixed link and added .html in: {filepath}")
                    files_updated += 1
