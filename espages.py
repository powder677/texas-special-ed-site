...
for district_folder in base_dir.iterdir():
    if district_folder.is_dir():
        ...
        for file_path in district_folder.glob("*.html"):
            ...
            # 3. Find the main content area (targets <article>, <main>, or .content-wrapper)
            content_area = soup.find("article") or soup.find("div", class_="content-wrapper")
            ...
            if content_area:
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    # prettify() can sometimes mess up text spacing, so we cast to str()
                    f.write(str(soup))
